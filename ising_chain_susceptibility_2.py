...
class IsingChainResult:
    ...
    chi_total: float
    correlation: np.ndarray | None = None        # shape (N,N)  <sigma_i sigma_j>, only if full_matrix=True
    susceptibility: np.ndarray | None = None      # shape (N,N)  chi_ij, only if full_matrix=True
...
def _offdiag_correlation_sum(
    N: int,
    M: list[np.ndarray],
    SigmaM: list[np.ndarray],
    P: list[np.ndarray],
    P_scale: list[float],
    Q: dict[int, np.ndarray],
    Q_scale: dict[int, float],
    log_Z: float,
) -> float:
    """
    Compute sum_{i<j} <sigma_i sigma_j> in O(N) time (instead of O(N^2)).

    Uses the recursion, for k = 1..N,

        W_k = W_{k-1} @ M_k  +  P_{k-1} @ SigmaM_k ,      W_0 = 0

    where W_k = sum_{i=1}^{k} P_{i-1} @ SigmaM_i @ (M_{i+1} ... M_k) accumulates,
    over every insertion point i <= k, the prefix product with sigma inserted
    at site i and carried forward through sites i+1..k. Then for each j,

        sum_{i<j} <sigma_i sigma_j> * Z = W_{j-1} @ SigmaM_j @ Q_{j+1}.

    Summing this over j = 1..N gives sum_{i<j} <sigma_i sigma_j> in a single
    O(N) sweep. W_{k-1} and P_{k-1} are added at potentially very different
    scales, so they are combined log-sum-exp style (aligned to their common
    max log-scale) before renormalizing, exactly like the prefix/suffix
    products elsewhere in this module.
    """
    W = np.zeros(2)
    W_scale = 0.0
    total = 0.0
    for j in range(1, N + 1):
        if j >= 2:  # W_0 = 0 contributes nothing when j == 1
            val = (W @ SigmaM[j - 1]) @ Q[j + 1]
            total += _ratio(val, W_scale + Q_scale[j + 1], log_Z)

        term_a = W @ M[j - 1]
        scale_a = W_scale
        term_b = P[j - 1] @ SigmaM[j - 1]
        scale_b = P_scale[j - 1]
        ref = max(scale_a, scale_b)
        combined = term_a * np.exp(scale_a - ref) + term_b * np.exp(scale_b - ref)
        W, ds = _normalize(combined)
        W_scale = ref + ds

    return total


def solve_ising_chain(
    J: float, h: np.ndarray, T: float, kB: float = 1.0, full_matrix: bool = True
) -> IsingChainResult:
    """
    Exactly solve the open 1D Ising chain with site-dependent field h (length N)
    via the transfer-matrix method, returning magnetization and chi_total
    (the fluctuation-dissipation total susceptibility).
    ...
    full_matrix : if True (default), also build the full N x N <sigma_i sigma_j>
        correlation matrix and chi_ij susceptibility matrix -- O(N^2) time/memory.
        If False, only chi_total is computed, in O(N) time/memory, via a
        cumulative-insertion recursion (no pairwise matrix is ever formed).
        Use False for long chains (thousands of sites) when only the scalar
        total susceptibility is needed.
    """
    ...
    if full_matrix:
        # ---------- correlations <sigma_i sigma_j>, all i,j (1-indexed internally), O(N^2) ----------
        ...
        return IsingChainResult(
            ...
            magnetization=magnetization, chi_total=chi_total,
            correlation=correlation, susceptibility=susceptibility,
        )

    # ---------------- chi_total only, O(N) time/memory (no N x N matrix formed) ----------------
    S_offdiag = _offdiag_correlation_sum(N, M, SigmaM, P, P_scale, Q, Q_scale, log_Z)
    # sum_{i,j} <sigma_i sigma_j> = N (diagonal, sigma_i^2=1) + 2*S_offdiag (each pair counted twice)
    sum_corr = N + 2.0 * S_offdiag
    chi_total = beta * (sum_corr - float(np.sum(magnetization)) ** 2)
...
def _self_test(seed: int = 0, trials: int = 25, max_N: int = 8) -> None:
    ...
    for t in range(trials):
        ...
        tm = solve_ising_chain(J, h, T, full_matrix=True)
        fast = solve_ising_chain(J, h, T, full_matrix=False)
        ...
        assert np.isclose(exact.chi_total, tm.chi_total, atol=1e-6), (t, "chi_total (full_matrix)")
        assert np.allclose(fast.magnetization, tm.magnetization, atol=1e-8), (t, "magnetization (fast)")
        assert np.isclose(exact.chi_total, fast.chi_total, atol=1e-6), (t, "chi_total (fast, O(N))")
    print(f"Self-test passed: {trials} random trials (N<={max_N}) match brute force exactly "
          f"(both full_matrix=True and the O(N) fast chi_total path).")
...
if __name__ == "__main__":
    ...
    # Fast-path benchmark: chi_total only, O(N), for a long chain.
    import time

    N_big = 5000
    rng = np.random.default_rng(0)
    h_big = rng.uniform(-2.0, 2.0, size=N_big)
    t0 = time.time()
    fast_result = solve_ising_chain(J=1.0, h=h_big, T=2.0, full_matrix=False)
    dt = time.time() - t0
    print(f"\nFast path, N={N_big}: chi_total = {fast_result.chi_total:.4f}  (took {dt:.3f}s)")
